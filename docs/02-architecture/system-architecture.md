...
M3's internal session controller now owns one direct agent subprocess with an absolute executable and working directory, an explicit non-inherited environment, strict protocol order, bounded diagnostics, cancellation, and wait cleanup. Tool manifests remain inert and do not cause a command to run. Protocol frames are strict UTF-8 JSON Lines objects capped at 1 MiB. This process lifecycle is not filesystem, network, syscall, or descendant-process isolation, and no supported public agent-run command exists yet. Deterministic decisions and monotonic event sequences remain independent from subprocess logging and wall-clock order. See [ADR-0011](decisions/0011-versioned-agent-json-lines-protocol.md) and [ADR-0012](decisions/0012-owned-agent-subprocess-sessions.md).
