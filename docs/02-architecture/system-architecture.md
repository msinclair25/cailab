...
M3's public `agent validate` and `agent run` workflows load bounded policy/tool documents, bind them to the active canonical scenario, and launch either the deterministic reference agent or an explicitly configured protocol-compatible subprocess. Protocol 1.1 makes the requested action and resource identifier explicit; the controller supplies canonical tenant and classification metadata instead of trusting the agent. Draft 2020-12 validation rejects invalid input and external schema references before launch. Direct allow/redact decisions reach a registered one-shot tool subprocess; approval-required calls remain unexecuted until a default rejection or exact local confirmation is re-evaluated and durably linked. Successful output is protected before return. SQLite stores immutable start and terminal run records and verifies canonical decision/approval/outcome evidence on read. `agent replay` revalidates those links and deterministically projects a complete compatible trial set into transparent counts and rates without executing the agent, tools, providers, policy, or a model. Host agent and tool subprocesses remain unisolated. The opt-in Docker agent adapter instead uses a content-addressed image with no host mounts or forwarded environment, `--network none`, a read-only root and bounded tmpfs, non-root execution, dropped capabilities, built-in seccomp, resource limits, and ownership-checked cleanup; tools still execute on the trusted host side. See [ADR-0011](decisions/0011-versioned-agent-json-lines-protocol.md), [ADR-0012](decisions/0012-owned-agent-subprocess-sessions.md), [ADR-0013](decisions/0013-deterministic-tool-policy-and-evidence.md), [ADR-0014](decisions/0014-strict-one-shot-tool-execution.md), [ADR-0015](decisions/0015-scenario-bound-public-agent-runs.md), [ADR-0016](decisions/0016-immutable-approval-resolution.md), [ADR-0017](decisions/0017-opt-in-docker-agent-isolation.md), and [ADR-0018](decisions/0018-deterministic-agent-evidence-replay.md).
