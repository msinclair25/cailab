...
M3's internal session controller owns one direct agent subprocess with explicit launch authority and bounded protocol resources. A versioned exact-match policy evaluator applies manifest ceilings and deny-first decisions. Draft 2020-12 validation rejects invalid input and external schema references before launch. Only allow/redact decisions reach the one-shot tool subprocess protocol; successful output is canonicalized and applies manifest sensitive-field pointers before return. The gateway commits authorization intent before execution and a linked succeeded/failed outcome before returning the result. SQLite verifies canonical hashes and decision chains on read. No supported public registration/run command exists yet, and neither subprocess controller enforces filesystem, network, syscall, or descendant isolation. See [ADR-0011](decisions/0011-versioned-agent-json-lines-protocol.md), [ADR-0012](decisions/0012-owned-agent-subprocess-sessions.md), [ADR-0013](decisions/0013-deterministic-tool-policy-and-evidence.md), and [ADR-0014](decisions/0014-strict-one-shot-tool-execution.md).
