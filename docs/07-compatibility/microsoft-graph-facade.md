...
CloudAILab supports only the operations below for the `microsoft-consent` scenario. The facade is shaped like a narrow Microsoft Graph v1.0 surface; it does not claim to emulate Microsoft Entra ID, token issuance, role enforcement, replication, throttling, or the complete OData query language.

The authoritative tests are `TestMicrosoftFacadeAuthPaginationAndGrantDeletion`, `TestMicrosoftSnapshotTracksLivePermissionGrants`, and the cross-platform `TestMicrosoftNativeIntegration` in [`internal/provider`](../../internal/provider).
...
| `servicePrincipals` | `GET /v1.0/servicePrincipals` and `GET /v1.0/servicePrincipals/{id}` | API-shaped, training-only | Returns `id`, `appId`, and `displayName`. | No app-role assignments, owners, credentials, or service-principal mutation. |
...
- Local OIDC discovery, JWKS, signed claims, expiry, rotation, and caller-specific authorization remain M2 work.
